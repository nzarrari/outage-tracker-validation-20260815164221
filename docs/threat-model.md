...
| # | Threat | Asset | Status | Mitigation |
|---|---|---|---|---|
| **D1** | **No rate limiting on any endpoint. A single client can hammer `/outages/by-region` (or any endpoint) and pin the B1 App Service Plan CPU, starving legitimate operators.** | **Availability** | **Exposed — HIGH** | **Add ASP.NET Core rate limiting (`Microsoft.AspNetCore.RateLimiting`). Start with `/outages/by-region`: fixed window, 10 requests/minute per client IP, 429 with `Retry-After: 60`. `OutageTracker.Api/Program.cs`. → This is the M6 Part B mitigation.** |
